...
1. 下载 `picoclaw-magisk-v0.4.4.zip`
...
**版本**: v0.4.4 | **基于**: sipeed/picoclaw v0.2.7 | **构建**: Go 1.25+

> 注: 上游 v0.2.9 已移除 `picoclaw-launcher-tui`,本模块使用 v0.2.7 以保留 tui 组件

## 🔒 安全提醒

- ⚠️ **本模块的 `config.json` 仅包含占位符**(如 `PLEASE_FILL_YOUR_API_KEY`),请勿直接使用
- ✅ 安装后在 `/sdcard/picoclaw/config.json` 填入真实 API key
- ✅ 用户的 config.json 已被 `.gitignore` 排除,不会被 git 跟踪
- ✅ 启动时若检测到占位符,服务会拒绝启动并提示错误

## 🛠️ 维护

```bash
# 从上游同步最新版本并重新打包
./scripts/release.sh v0.2.7       # 使用上游 v0.2.7
./scripts/release.sh v0.2.7 v0.5.0 # 指定新模块版本
```
